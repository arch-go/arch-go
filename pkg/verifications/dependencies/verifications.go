...
func CheckRules(moduleInfo model.ModuleInfo, functionRules []*config.DependenciesRule) *RulesResult {
	result := &RulesResult{
		Passes: true,
	}
...
func CheckRule(moduleInfo model.ModuleInfo, rule config.DependenciesRule) *RuleResult {
	result := &RuleResult{
		Rule:        rule,
...
func checkDependencies(pkg *model.PackageInfo, rule config.DependenciesRule, moduleInfo model.ModuleInfo) (bool, []string) {
...
func checkAllowedDependencies(pkg *model.PackageInfo, rule config.DependenciesRule, moduleInfo model.ModuleInfo) (bool, []string) {
	result := true
	var details []string
...
func checkRestrictedDependencies(pkg *model.PackageInfo, rule config.DependenciesRule, moduleInfo model.ModuleInfo) (bool, []string) {
	result := true
	var details []string
